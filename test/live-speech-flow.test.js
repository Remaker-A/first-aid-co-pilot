...
test("段①②③ 四段主线贯通：session_started -> S1 -> S2 -> S3 -> 自动连跳停在 S6", async () => {
  ...
  assertIncludes(s6.tts, ["疑似", "120", "胸口中央", "说开始"], "S6 bridge TTS");
  assert.equal(s4.hasAudioBegin, false, "S4 should defer spoken audio to the bridge");
  assert.equal(s5.hasAudioBegin, false, "S5 should defer spoken audio to the bridge");
  assert.ok(s6.hasAudioBegin && s6.hasAudioEnd, "S6 bridge streams the single spoken audio");
...
test("自动连跳有界且停在 S6：恰好三段 guidance/state/audio、不越界进 S7", async () => {
  ...
  // Three guidance events but one bridged audio envelope within a single shared turn_seq.
  ...
  assert.equal(auto.json.filter((event) => event.type === "audio_begin").length, 1);
  assert.equal(auto.json.filter((event) => event.type === "audio_end").length, 1);
  assert.equal(auto.audio.length, 1, "one PCM frame for the bridged spoken segment");
  assert.deepEqual(
    auto.segments.map((segment) => segment.hasAudioBegin),
    [false, false, true],
    "only the final S6 bridge should speak"
  );
...
test("S7 说 AED 到了 -> 进入 S8 并播 AED 分析/电击安全指令", async () => {
  const scenario = buildScenario("live_flow_aed_arrival");
  await driveToCprReady(scenario);
  await scenario.drive({ type: "final", text: "开始" });

  const aed = await scenario.drive({ type: "final", text: "AED 到了" });
...
test("S7 说 120 到了 -> 进入 S9 交接并停止节拍", async () => {
  const scenario = buildScenario("live_flow_ems_arrival_120");
  await driveToCprReady(scenario);
  await scenario.drive({ type: "final", text: "开始" });

  const ems = await scenario.drive({ type: "final", text: "120 到了" });
  assert.equal(ems.finalEvent.intent, "paramedics_arrived");
  assert.equal(ems.finalStage, "S9_HANDOVER");
  assert.equal(ems.segments[0].intent, "generate_handover_report");
  assert.ok(ems.segments[0].toolTypes.includes("stop_haptic_metronome"));
  assert.ok(ems.segments[0].toolTypes.includes("generate_handover_report"));
  assertIncludes(ems.segments[0].tts, ["急救员到达", "交接报告"], "EMS handover TTS");
...
// P2-7 归因回归：在 S6 说"开始按压/继续按/开始 CPR/开始按压吧"——这些被 classifyIntent
// 判成 continue_cpr、但不被窄就绪正则识别的话术——同样应归因到 rule_flow_fast_path，
// 而非漂成 state_machine_critical（行为/话术一致，仅来源归因被修正）。
const CONTINUE_CPR_START_PHRASES = ["开始按压", "继续按", "开始 CPR", "开始按压吧"];
for (const phrase of CONTINUE_CPR_START_PHRASES) {
  test(`P2-7 归因：S6 说"${phrase}"（continue_cpr 非就绪正则）仍归因 rule_flow_fast_path`, async () => {
    const scenario = buildScenario(`live_flow_continue_${phrase}`);
    await driveToCprReady(scenario);

    const start = await scenario.drive({ type: "final", text: phrase });
    // Behaviour stays correct: flips to S7 and starts the loop in one segment.
    assert.equal(start.finalStage, "S7_CPR_LOOP", `"${phrase}" must flip S6 -> S7`);
    assert.equal(start.segments.length, 1, `"${phrase}" is a single start segment`);
    assert.equal(start.finalEvent.intent, "continue_cpr", `"${phrase}" resolves to continue_cpr`);
    assert.ok(
      ["continue_cpr", "start_cpr_loop"].includes(start.segments[0].intent),
      `"${phrase}" guidance starts the loop (got: ${start.segments[0].intent})`
    );
    // The fix: source is the deterministic flow fast path, never state_machine_critical.
    assert.equal(
      start.segments[0].source,
      "rule_flow_fast_path",
      `"${phrase}" source must be rule_flow_fast_path, not drift to state_machine_critical`
    );
    assertIncludes(start.segments[0].tts, ["开始按压", "跟着节拍", "用力快压"], `"${phrase}" startup TTS`);
    assert.equal(start.segments[0].state?.cpr_state?.started, true, `"${phrase}" marks cpr_state.started`);
