...
class LiveFastIntentRouterTest {
    ...
    fun cprHotQuestionsProduceIntentHintsOnly() {
        ...
        val zhAedArrival = inferLiveFastIntent("自动体外除颤器到了")
        ...
        assertEquals("aed_available", zhAedArrival?.intent)
        ...
        assertTrue((zhAedArrival?.confidence ?: 0.0) > 0.8)
    ...
    fun cprHotQuestionsCoverNaturalStopAndAedAlternationVariants() {
        ...
        assertEquals("continue_cpr", inferLiveFastIntent("我好了")?.intent)
        assertEquals("continue_cpr", inferLiveFastIntent("准备好了可以开始")?.intent)
        assertEquals("continue_cpr", inferLiveFastIntent("来吧")?.intent)
        assertEquals("continue_cpr", inferLiveFastIntent("开始压吧")?.intent)
